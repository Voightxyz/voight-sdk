...
export interface DetectedProviders {
  ...
  /**
   * Version range of the `ai` package (Vercel AI SDK), or `null`.
   * When present we treat the project as a Vercel AI SDK app and
   * route the wizard down the OpenTelemetry-exporter path instead
   * of the direct-wrapper path — `streamText` / `generateText` /
   * `streamObject` / `generateObject` calls are caught at the OTel
   * layer rather than by wrapping the underlying provider client.
   */
  vercel: string | null
}
 ...
 * collapse to `{ openai: null, anthropic: null, vercel: null }` so
 * the caller gets a uniform shape regardless of failure mode.
 *
...
export function detectAppProviders(cwd: string): DetectedProviders {
  ...
  if (!existsSync(pkgPath)) {
    return { openai: null, anthropic: null, vercel: null }
  }
  try {
    ...
    const anthropic =
      typeof deps['@anthropic-ai/sdk'] === 'string'
        ...
        : null
    const vercel = typeof deps['ai'] === 'string' ? deps['ai'] : null
...
/**
 * Build the install command for the Vercel AI SDK path. We list the
 * Voight exporter and `@vercel/otel`. The `ai` package and a
 * provider package (`@ai-sdk/openai` etc.) are already installed
 * (that's how we detected this project as Vercel AI SDK), so we
 * don't re-list them — keeps the post-install command focused on
 * what's actually new to the project.
 */
export function packageManagerInstallCommandVercel(
  pm: PackageManager,
): string {
  const pkgs = '@voightxyz/vercel-ai @vercel/otel'
  if (pm === 'pnpm') return `pnpm add ${pkgs}`
  if (pm === 'yarn') return `yarn add ${pkgs}`
  if (pm === 'bun') return `bun add ${pkgs}`
  return `npm install ${pkgs}`
...
/**
 * Generate the contents of `instrumentation.ts` for the Vercel AI
 * SDK path. The file registers `@vercel/otel` with `VoightExporter`
 * as the trace exporter — every `streamText` / `generateText` /
 * `streamObject` / `generateObject` call that flips
 * `experimental_telemetry: { isEnabled: true }` lands in Voight.
 *
 * No edits to `generateAppVoightModule` above — this is a parallel
 * code path for Vercel AI SDK projects. A project that has only
 * `openai` / `@anthropic-ai/sdk` (no `ai`) never reaches this
 * function and gets the wrapper-based module exactly as in 0.6.4.
 */
export function generateInstrumentationModule(opts: {
  agentName: string
  privacy: PrivacyLevel
}): string {
  const { agentName, privacy } = opts
  return [
    '// ──────────────────────────────────────────────────────────────────',
    '// Voight observability — generated by `npx @voightxyz/sdk init`',
    '//',
...
/**
 * Where the Vercel AI SDK path writes the OpenTelemetry registration.
 * Next.js looks for `instrumentation.ts` in two places: the project
 * root and `src/instrumentation.ts` (only when `src/` exists, since
 * Next.js 13.4). We mirror that — if the project uses `src/` for
 * everything else, the file goes there too; otherwise root.
 *
 * See: https://nextjs.org/docs/app/building-your-application/optimizing/instrumentation
 */
export function resolveInstrumentationPath(cwd: string): {
  fullPath: string
  display: string
} {
  const hasSrc = existsSync(join(cwd, 'src'))
  const display = hasSrc ? 'src/instrumentation.ts' : 'instrumentation.ts'
  return { fullPath: join(cwd, display), display }
}
...
function printDetectionSummary(
  ...
  if (providers.vercel) detected.push(`ai (${providers.vercel})`)
  ...
  if (providers.vercel && (providers.openai || providers.anthropic)) {
    // Vercel AI SDK wins when both are present — the OTel exporter
    // covers every streamText / generateText / streamObject /
    // generateObject call regardless of underlying provider, and
    // we'd double-instrument if we also generated wrapped clients.
    console.log(
...
function printVercelUsageSnippet(framework: Framework): void {
  // Vercel AI SDK path: the user already wires their calls — we
  // just need to remind them to flip `experimental_telemetry` on
  // and (optionally) pass per-user metadata.
  console.log('  Activate telemetry on your existing calls:')
  console.log('')
  if (framework === 'next') {
    console.log(`    // app/api/chat/route.ts`)
    ...
    console.log(`        prompt: (await req.json()).prompt,`)
    console.log(`        experimental_telemetry: {`)
    console.log(`          isEnabled: true,`)
    console.log(`          metadata: { userId: 'user_123', plan: 'pro' },`)
    console.log(`        },`)
    console.log(`      })`)
    console.log(`      return result.toAIStreamResponse()`)
    console.log(`    }`)
  } else {
    console.log(`    import { openai } from '@ai-sdk/openai'`)
    console.log(`    import { streamText } from 'ai'`)
    ...
    console.log(`      experimental_telemetry: {`)
    console.log(`        isEnabled: true,`)
    console.log(`        metadata: { userId: 'user_123' },`)
    console.log(`      },`)
    console.log(`    })`)
  }
  console.log('')
...
export async function runInit(argv: string[]): Promise<void> {
  ...
  const hasOpenai = providers.openai !== null
  const hasAnthropic = providers.anthropic !== null
  const hasVercel = providers.vercel !== null
  if (!hasOpenai && !hasAnthropic && !hasVercel) {
    console.error(
      '  ✗ No openai, @anthropic-ai/sdk, or ai (Vercel AI SDK) detected in package.json.',
    )
    console.error('')
    console.error(
    ...
    console.error('    Otherwise, install the SDK you plan to use:')
    console.error('')
    console.error('      npm install openai')
    console.error('      npm install @anthropic-ai/sdk')
    console.error('      npm install ai @ai-sdk/openai          # Vercel AI SDK')
    console.error('')
  ...
  // ── Generate & write the target module ──────────────────────────
  //
  // Two paths share everything above (detection, privacy, key,
  // agent name) and split here. The Vercel AI SDK path writes a
  // root-level `instrumentation.ts` registering `VoightExporter`
  // with `@vercel/otel`; the direct-wrapper path keeps the original
  // 0.6.4 behaviour and writes `src/lib/voight.ts` with wrapped
  // OpenAI / Anthropic clients. The branch is taken purely on
  // whether `ai` was detected — projects without `ai` get the
  // wrapper module byte-identically to 0.6.4.
  if (hasVercel) {
    const { fullPath, display } = resolveInstrumentationPath(cwd)
    ...
    if (existsSync(fullPath)) {
      ...
      console.error(
        '    If you wire OTel manually, you can also add VoightExporter to your',
      )
      console.error(
        '    existing `MultiSpanProcessor` setup — see https://docs.voight.xyz/ai-apps/vercel-ai',
      )
      process.exit(1)
    }

    const moduleSource = generateInstrumentationModule({
      agentName,
      privacy,
    })

    const parentDir = fullPath.slice(0, fullPath.lastIndexOf('/'))
    if (parentDir !== cwd && !existsSync(parentDir)) {
  ...
  if (hasVercel) {
    printVercelUsageSnippet(framework)
    console.log('  One more step — install the Voight exporter:')
    console.log(`    ${packageManagerInstallCommandVercel(pm)}`)
  } else {
